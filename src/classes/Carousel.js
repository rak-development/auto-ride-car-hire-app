export class Carousel {
   ...
   */
  #setPointerEvents() {
    this.#mainContainer.addEventListener('pointerdown', (e) => this.#handlePointerDown(e))
    this.#mainContainer.addEventListener('pointerup', (e) => this.#handlePointerUp(e))
  }
  ...
  #setDotsNavigation() {
    ...
    this.#slidesData.forEach((slide, i) => {
      ...
      if (i === this.#currentIndex) button.classList.add(this.#classNames.active)
  ...
  #handlePointerDown({ clientX }) {
    if (this.#autoTransitionIntervalId) clearInterval(this.#autoTransitionIntervalId)
  ...
  #handlePointerUp({ clientX }) {
    ...
    const reachedThreshold = Math.abs(clientX - this.#swipe.offset) >= this.#swipe.threshold
  ...
  #setCurrentSlide(index) {
    ;[...this.#mainContainer.children].forEach((slide) => {
      if (+slide.dataset.slide === index) slide.classList.add(this.#classNames.active)
   ...
   */
  #setActiveDot(index) {
    const dots = this.#mainContainer.querySelector(`.${this.#classNames.navigation.dots}`)
  ...
  #setOffsetCustomProperty(value) {
    this.#mainContainer.style.setProperty('--revealSlideOffset', `${value}px`)
  }
  ...
  #setSizeCustomProperties() {
    this.#mainContainer.style.setProperty('--slider-width', this.#configuration.width)
    this.#mainContainer.style.setProperty('--slider-height', this.#configuration.height)
  }
}
